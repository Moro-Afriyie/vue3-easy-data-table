...
export default function useHeaders(
	...
	const sortCycle: (SortType | 'none')[] = ['desc', 'asc', 'none'];
	const previousSortType = ref<SortType | 'none'>('none');
	const isInitialSort = ref(true);

	const updateSortField = (newSortBy: string, oldSortType: SortType | 'none') => {
		let newSortType: SortType | 'none' = 'none';

		console.log('odlSortType', oldSortType);

		let currentIndex = sortCycle.indexOf(oldSortType);

		if (previousSortType.value === oldSortType) {
			if (oldSortType === 'none') {
				newSortType = 'desc';
			} else if (oldSortType === 'asc') {
				newSortType = null;
			} else if (oldSortType === 'desc') {
				newSortType = 'asc';
			}
		} else {
			const nextIndex = (currentIndex + 1) % sortCycle.length;
			newSortType = sortCycle[nextIndex] === 'none' ? null : sortCycle[nextIndex];

			if (mustSort && newSortType === 'none') {
				newSortType = 'asc';
			}

			previousSortType.value = newSortType;
		}
